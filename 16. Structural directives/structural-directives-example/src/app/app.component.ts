...
@Component({
  ...
  template: `
    <main>
      ...
      <section *hideAfter="7 as time; then placeholder; let counter = counter; let default" class="banner primary">
      ...
      <ng-template #placeholder let-hiddenAfter="hideAfter" let-myDefault>
        <section  class="banner secondary">
          <h2>Placeholder</h2>
          <p>Here was some content, It was visible for {{hiddenAfter}} seconds. Default is {{myDefault}}</p>
        </section>
      </ng-template>
      
      <section *ifLoaded="state">Data was loaded. User name: {{state.data.name}}</section>
